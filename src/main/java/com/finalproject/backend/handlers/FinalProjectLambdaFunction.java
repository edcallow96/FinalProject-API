...
public class FinalProjectLambdaFunction implements Function<S3Event, S3Event> {
  ...
  public FinalProjectLambdaFunction(final AmazonS3 amazonS3, final ProducerTemplate producerTemplate, final CamelContext camelContext) {
  ...
  public S3Event apply(S3Event s3Event) {
    ...
    for (S3EventNotification.S3EventNotificationRecord record : s3Event.getRecords()) {
      ...
      try {
        ...
        producerTemplate.send(ENTRY_POINT_ROUTE, ExchangeBuilder.anExchange(camelContext).withBody(s3Object).build());
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
